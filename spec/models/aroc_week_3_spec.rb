...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
      users = User.distinct.joins(:order_items).where("item_id = #{@item_8.id}").order('name').pluck('name')
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # Solution goes here
      order1 = Order.last
      names = Item.joins(:order_items).where("order_id = #{order1.id}").pluck('items.name')
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
      order3 = Order.where("user_id = #{@user_3.id}").limit(3).last
      items_for_user_3_third_order = Item.joins(:order_items).where("order_id = #{order3.id}").pluck('name')
    # ------------------------------------------------------------
